...
//
// ВАЖНАЯ НАХОДКА (сверено на реальной установке 2026-07-08): пакет
// embedded-postgres (windows-x64) поставляет только СЕРВЕРНЫЕ бинарники
// (initdb.exe/pg_ctl.exe/postgres.exe), но НЕ клиентские — pg_dump.exe в
// комплекте НЕТ. Поэтому этот бэкап пока gracefully пропускается (findPgBinDir
// в main.js вернёт native/bin, но pg_dump.exe там не окажется). Варианты на
// будущее: (1) доложить pg_dump.exe из полной сборки Postgres в extraResources;
// (2) логический дамп через node-postgres (getPgClient); (3) использовать
// готовый JSON-экспорт компании из API (/backup/export). Пока — не блокер: без
// бэкапа коробка работает, а данные не теряются (persistent pgdata в userData).
//
