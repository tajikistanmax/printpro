...
- ✅ ~~Иконка приложения~~ — **сделано:** `electron/build/icon.ico` (логотип PrintPro).
- ✅ ~~Порт-конфликты — тихий полу-запуск~~ — **сделано:** понятная ошибка (см. выше).
- ✅ ~~Бэкап данных~~ — **сделано:** холодная копия pgdata + ротация (см. выше).
- Канал автообновлений (`publish` в package.json) — требует решения владельца:
  GitHub Releases (публичный репо релизов) или свой сервер обновлений. Без него
  фикс-версии клиентам придётся раздавать вручную.
- Подпись установщика (code signing) — без неё Windows SmartScreen/антивирус
  предупреждает «неизвестный издатель» при установке у клиента.
