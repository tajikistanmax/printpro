...
> (3000), web (3001) и встроенный Postgres (5433). Если порт занят (например, на
> машине разработчика запущен другой Next.js-проект на 3000/3001) — API/web не
> стартуют (`EADDRINUSE`). На чистой машине клиента порты свободны. *На будущее
> (по желанию):* сделать порты настраиваемыми/авто-подбор, чтобы коробка не
> конфликтовала с другим ПО на ПК клиента.

## Остаётся по желанию (не блокеры)
- ✅ ~~Иконка приложения~~ — **сделано:** `electron/build/icon.ico` (логотип PrintPro,
  16–256px), прописана в `win.icon` + иконки NSIS-установщика.
