...
| `preload.js` | Минимальный мост (`contextIsolation: true`) для экрана настройки: `window.electronAPI.{getConfig, getLanIp, saveConfig, getBackupInfo, pickBackupDir, restoreAtSetup}`. |
...
- при **запуске** (до старта Postgres) — не чаще раза в 6ч, чтобы не изнашивать флешку;
- при **выходе** из программы (после остановки Postgres) — всегда (свежая копия конца дня);
- по кнопке **«Сделать резервную копию сейчас»** (меню «PrintPro» на главном ПК) —
  Postgres на пару секунд останавливается, копия делается, Postgres снова
  поднимается; при сбое подъёма — приложение перезапускается (надёжное восстановление).
