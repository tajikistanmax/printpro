...
| `preload.js` | Минимальный мост (`contextIsolation: true`): `window.electronAPI.{getConfig, getLanIp, saveConfig, getBackupInfo, pickBackupDir, restoreAtSetup, notifyShiftClosed}`. `notifyShiftClosed` зовёт веб-панель при закрытии смены (в облаке метода нет). |
| `setup.html` / `setup.js` | Экран первого запуска: выбор роли ПК («Главный компьютер» / «Касса»), адрес главного ПК, галочка «Синхронизировать с облаком», **папка для резервных копий + восстановление из копии** (для роли «главный»). Отдельный экран Electron, НЕ часть `printpro-web`. |
| `backup.js` | Резервные копии базы без `pg_dump`: холодная копия папки `pgdata` (`coldCopy`), ротация, восстановление (`restore`) с проверкой версии Postgres. См. раздел «Защита данных». |
...
3. **Главный ПК** (`role: 'main'`):
   ...
   - делает резервную копию базы (при запуске — до старта Postgres; при выходе —
     после остановки; локально + во внешнюю папку, если задана). См. раздел
     «Защита данных».
...
## Уровень 2 — сервер как служба Windows (главная проблема: «закрыл главный → кассы отвалились»)

**Проблема.** Раньше сервер (Postgres+API+веб) жил ВНУТРИ окна главного ПК:
закрыл окно → `before-quit` гасил базу/API/веб → кассы теряли подключение.

**Решение.** Сервер вынесен в отдельный headless-процесс, который запускается
как **служба Windows** (автостарт при загрузке ПК, до входа пользователя, живёт
независимо от окна). Кассы работают, пока главный ПК включён — окно можно
закрывать.

**Из чего состоит (все файлы — в `electron/`):**

| Файл | Роль |
|---|---|
| `data-paths.js` | Единый путь к данным `C:\ProgramData\PrintPro` для окна И службы (служба работает от SYSTEM, окно — от пользователя; общая папка обязательна). Плюс общий `config.json`. |
| `server-host.js` | **Headless-сервер.** Плоский Node (запускается как `PrintPro.exe` с `ELECTRON_RUN_AS_NODE=1`) — без окна/Chromium, поэтому надёжен в сессии 0. Поднимает Postgres+миграции+seed+API+веб, делает копии, слушает управляющий эндпоинт `127.0.0.1:3002`. |
| `service/PrintProService.xml` | Конфиг WinSW (обёртки, делающей службу). |
| `service/PrintProService.exe` | **Кладётся вручную один раз** — сам WinSW (см. `service/README.md`). В git не коммитится. |
| `build/installer.nsh` | Шаги установщика: создать ProgramData + права (icacls), перенести старую базу, поставить и запустить службу (только на «главном» ПК). |

**Как работает запуск (main.js, роль «главный»):**
1. Окно стучится на `127.0.0.1:3002` (служба жива?).
2. **Служба есть** → окно НЕ поднимает сервер, просто открывает `http://localhost:3001`. Закрытие окна сервер не трогает.
3. **Службы нет** → запасной «встроенный» путь: окно само поднимает сервер (как до Уровня 2). Ничего не ломается на установках без службы.

Резервные копии при живой службе делает служба (она владеет базой): окно/веб
просят её через `POST /control/backup-now` (кнопка «копия сейчас», закрытие смены).

**Переезд данных.** База/загрузки/настройки переезжают из профиля пользователя
(`%APPDATA%`) в `C:\ProgramData\PrintPro`. Перенос — ОДИН раз, КОПИРОВАНИЕМ (старое
остаётся как страховка): в двух местах для надёжности — в установщике (NSIS, до
первого старта службы) и в `main.js` (`migrateUserDataToProgramDataOnce`, страховка
для реального пользователя окна).

### Сборка Уровня 2 (что сделать перед `npm run dist`)

1. **Положить WinSW** — скачать `WinSW-x64.exe` (v2.x) с
   https://github.com/winsw/winsw/releases, переименовать в `PrintProService.exe`,
   положить в `electron/service/`. Подробно — в [`service/README.md`](service/README.md).
2. Собрать api и web (`npm run build` в обеих папках), `npm install` в `electron/`.
3. `npm run dist` (с зеркалом electron-builder, если GitHub CDN недоступен).

### Проверка headless-сервера ОТДЕЛЬНО (до службы, снимает главный риск)

На главном ПК из папки `electron/` (api и web должны быть собраны):

```powershell
$env:ELECTRON_RUN_AS_NODE = "1"
$env:PRINTPRO_RESOURCES = (Resolve-Path "..").Path
.\node_modules\electron\dist\electron.exe server-host.js
```

Ожидаемо: в консоли Postgres → миграции → seed → API → веб → «server-host готов».
Затем `http://localhost:3001` открывает панель. `Ctrl+C` — корректная остановка.
Логи: `C:\ProgramData\PrintPro\logs\server.log`.

### Ограничение (известное, для след. итерации)
Восстановление из копии при РАБОТАЮЩЕЙ службе пока делается вручную (остановить
службу → открыть PrintPro без службы → «Восстановить из копии…» → снова запустить
службу). В окне при serviceMode на эту кнопку показывается инструкция. Полное
восстановление через управляющий эндпоинт службы — следующий шаг.
...
8. **`embedded-postgres` — API сверен ✅, бинарники под Windows.** API пакета
   ...
   **Версия:** пакет публикует ТОЛЬКО бета-версии, привязанные к мажору
   ...
   `postgres.exe`), клиентского `pg_dump.exe` нет (в `native/bin` — только эти три,
   перепроверено 2026-07-12). Поэтому бэкап реализован как **холодная копия папки
   `pgdata`** (без внешних утилит) + восстановление — см. раздел «Защита данных».
   Логический дамп на будущее: `pg_dump.exe` в extraResources или JSON-экспорт из
   API (`GET /api/backup/export`). Не блокер: данные в persistent `pgdata` не теряются.
...
## Защита данных: бэкапы + внешняя папка + восстановление ✅ (2026-07-12)

`pg_dump.exe` в комплект `embedded-postgres` **не входит** (в `native/bin` только
серверные `initdb`/`pg_ctl`/`postgres`; проверено), поэтому копия — **холодная
физическая копия папки данных** Postgres (`pgdata`), снятая, когда сервер
**остановлен** (кластер не запущен → копия консистентна, без внешних утилит).
Логика — в `backup.js` (`coldCopy`/`restore`/ротация), оркестрация — в `main.js`
(`runBackups`).

**Куда пишутся копии (главное — защита от переустановки Windows):**
- **Локально**, `userData/backups/pgdata_<дата>` (диск C) — страховка от порчи
  базы при живом диске. Хранится 5 копий.
- **Во внешнюю папку** (флешка / диск D / другой носитель), которую владелец
  указывает в настройке `backupDir`. Хранится 10 копий. **Именно эта копия
  переживает переустановку Windows** — диск C форматируется вместе с базой И
  локальными копиями, уцелеет только внешняя.

Внутри каждой копии — манифест `printpro-backup.json` (версия программы, мажор
Postgres, дата). Восстановление сверяет мажор Postgres и отказывается с понятной
причиной при несовпадении.

**Когда делаются копии** (все — в консистентный момент, база остановлена):
- **ОСНОВНАЯ — по закрытию смены (Z-отчёт).** Веб-панель после успешного закрытия
  смены зовёт `window.electronAPI.notifyShiftClosed()` (в облаке метода нет — no-op,
  принцип «один код»). Коробка через ~4с (чтобы веб успел обновиться) делает копию
  `stop→copy→start`. Смену закрывают раз в день в конце дня → копия примерно раз в
  день, с полными данными за день, **независимо от того, сколько раз открывали/
  закрывали программу**. Повторные сигналы в пределах 10 мин игнорируются.
- **Подстраховка при запуске/выходе программы** — throttled, **не чаще раза в сутки**
  (`BACKUP_MIN_INTERVAL_MS`). Раньше копия делалась на каждом закрытии программы —
  при частом открытии/закрытии это плодило копии и съедало историю ротацией;
  теперь это лишь страховка на случай, если смену забыли закрыть.
- по кнопке **«Сделать резервную копию сейчас»** (меню «PrintPro» на главном ПК) —
  тот же `stop→copy→start`; при сбое подъёма базы — приложение перезапускается
  (надёжное восстановление). Ручная копия и копия по смене используют общий
  механизм `safeBackupWhileRunning()` (не запускаются две сразу).

**Восстановление — одной кнопкой** (не «руками подменять папку»):
- на **главном ПК**: меню «PrintPro» → «Восстановить из копии…» → выбрать папку
  `pgdata_…` → подтвердить → программа перезапустится на восстановленной базе.
  Прежняя база сохраняется рядом как `pgdata.before-restore_<дата>` (страховка;
  при следующем старте оставляем только самую свежую страховочную).
- на **первом запуске после переустановки Windows**: на экране настройки —
  «Резервные копии базы» → «Восстановить из копии…» (укладывает копию в
  `userData/pgdata`, делает этот ПК главным, перезапускается).

**Настройка папки для копий:** экран настройки (роль «Главный компьютер») или
меню «PrintPro» → «Папка для копий…». Программа находит диски кроме C и предлагает
их; если выбрать папку на диске C — предупреждает. Если внешняя папка не задана —
при старте главного ПК показывается разовое предупреждение с предложением выбрать
флешку/диск.

- **Ограничение:** авто-копии делаются при запуске/выходе. Для конторы, которая
  открывается/закрывается ежедневно (обычный случай) — это ежедневная копия. Если
  ПК работает сутками без перезапуска — свежую копию можно снять кнопкой «Сделать
  резервную копию сейчас». *На будущее (по желанию):* докладка `pg_dump.exe` для
  «горячего» логического дампа без остановки и/или облачная выгрузка бэкапа
  (в API уже есть логическая выгрузка компании — `GET /api/backup/export`).
