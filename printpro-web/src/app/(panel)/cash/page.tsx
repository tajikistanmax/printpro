...
export default function CashPage() {
  ...
  if (!shift) {
    return (
      <div>
        <h1 className="mb-6 text-2xl font-bold text-slate-800 dark:text-slate-100">Касса</h1>
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-100">Касса</h1>
        <div className="flex items-center gap-3">
          <button
            onClick={() => setShowX(true)}
            className="rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-3 py-1.5 text-sm text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800"
          >
            🖨 X-отчёт
          </button>
          <span className="rounded-full bg-emerald-100 dark:bg-emerald-900/30 px-3 py-1 text-sm font-medium text-emerald-700 dark:text-emerald-300">
          ...
          <div className="w-80 rounded-2xl bg-white dark:bg-slate-900 p-6 shadow-2xl">
            ...
            <div className="no-print mt-5 flex gap-2">
              ...
              <button
                ...
                className="flex-1 rounded-lg border border-slate-200 dark:border-slate-700 py-2 text-sm text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800"
              >
                Закрыть
              </button>
            </div>
      ...
      <div className="mb-6 grid grid-cols-2 gap-3 md:grid-cols-4">
        {cards.map((c) => (
          <div key={c.label} className="rounded-2xl bg-white dark:bg-slate-900 p-4 shadow-sm">
      ...
      <div className="mb-6 grid gap-3 md:grid-cols-3">
        <div className="rounded-2xl bg-white dark:bg-slate-900 p-4 shadow-sm">
          <div className="text-sm text-slate-500 dark:text-slate-400">Выручка деньгами</div>
        ...
        <div className="rounded-2xl bg-white dark:bg-slate-900 p-4 shadow-sm">
          <div className="text-sm text-slate-500 dark:text-slate-400">Внесения / изъятия</div>
          <div className="text-xl font-bold text-slate-800 dark:text-slate-100">
      ...
      {canOperate && (
        <div className="mb-6 grid gap-4 md:grid-cols-2">
          ...
          <div className="rounded-2xl bg-white dark:bg-slate-900 p-5 shadow-sm">
            ...
            <form onSubmit={addMovement} className="flex flex-wrap items-end gap-3">
              <div className="min-w-[120px]">
                <label className="mb-1 block text-sm text-slate-500 dark:text-slate-400">Тип</label>
                <select
                  value={moveType}
              ...
              <div className="min-w-[120px]">
                <label className="mb-1 block text-sm text-slate-500 dark:text-slate-400">Сумма</label>
                <input
                  type="number"
                  step="0.01"
                  min="0.01"
              ...
              <div className="min-w-[160px] flex-1">
                <label className="mb-1 block text-sm text-slate-500 dark:text-slate-400">Причина</label>
                <input
                  value={moveReason}
          ...
          {/* Закрытие смены */}
          <div className="rounded-2xl bg-white dark:bg-slate-900 p-5 shadow-sm">
            <h2 className="mb-3 font-semibold text-slate-700 dark:text-slate-200">Закрытие смены</h2>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div className="rounded-2xl bg-white dark:bg-slate-900 p-5 shadow-sm">
          <h2 className="mb-3 font-semibold text-slate-700 dark:text-slate-200">Оплаты</h2>
          {shift.payments.length === 0 ? (
            <p className="text-sm text-slate-400 dark:text-slate-500">Оплат пока нет.</p>
          ) : (
        ...
        <div className="rounded-2xl bg-white dark:bg-slate-900 p-5 shadow-sm">
          <h2 className="mb-3 font-semibold text-slate-700 dark:text-slate-200">Движения денег</h2>
          {shift.movements.length === 0 ? (
            <p className="text-sm text-slate-400 dark:text-slate-500">Движений пока нет.</p>
          ) : (
            <div className="space-y-1.5">
              {shift.movements.map((m: any) => (
                ...
                >
                  <span className="text-slate-600 dark:text-slate-300">
                    {m.category && (
                      <span className="mr-1.5 rounded bg-slate-100 dark:bg-slate-700 px-1.5 py-0.5 text-xs text-slate-500 dark:text-slate-400">
                        {m.category}
                      </span>
                    )}
